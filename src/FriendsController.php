...
final class FriendsController
{
    ...
    { //carga la lista de amigos ya existentes
        ...
        $sql = "  SELECT f.id, " . "
                         ...
                         u.display_name, u.email, u.birth_date, u.preferred_language, f.created_at, f.updated_at
                  FROM friends f JOIN users u ON u.id = CASE
                                                          WHEN f.requester_user_id = :uid THEN f.addressee_user_id
                                                          ELSE f.requester_user_id
                                                        END
                  WHERE f.deleted_at IS NULL AND f.status = 'accepted' AND (:uid IN (f.requester_user_id, f.addressee_user_id))
    ...
    {//solicitudes de amistad enviadas y recibidas (pendientes)
        Middleware::requireAppSignature();
        $userId = Middleware::requireAuthUserId();
        ...
        $inSql = "SELECT f.id, f.requester_user_id AS from_user_id, u.display_name, u.email, f.status, f.created_at, f.updated_at " . "
                  FROM friends f JOIN users u ON u.id = f.requester_user_id
                  WHERE f.deleted_at IS NULL AND f.status = 'pending' AND f.addressee_user_id = :uid
        ...
        $outSql = "SELECT f.id, f.addressee_user_id AS to_user_id, u.display_name, u.email,f.status, f.created_at, f.updated_at " . "
                      FROM friends f JOIN users u ON u.id = f.addressee_user_id
                      WHERE f.deleted_at IS NULL AND f.status = 'pending' AND f.requester_user_id = :uid
                      ORDER BY f.created_at DESC ";
    ...
    {//crea una solicitud de amistad
        ...
        //si no hay ninguno de los datos parra ubicar al usuario destinatario
        ...
        //viene el correo del destinatario
        if ($toUserId <= 0 && $toEmail !== '') {
        ...
        // Verificar si ya existe solicitud/amistad en cualquiera de los sentidos (A->B o B->A)
        $check = $pdo->prepare("SELECT * " . " FROM friends " . "
        ...
        if ($existing) {
            // Si ya aceptada, no crear duplicado
            if ($existing['status'] === 'accepted') {
                Http::json(409, ['error' => 'conflict', 'message' => 'already_friends']);
                return;
            }
            // Si hay pending invertida (la otra persona te solicitó), podrías auto-aceptar (opcional).
            Http::json(409, ['error' => 'conflict', 'message' => 'request_already_exists', 'request' => $existing]);
            return;
        }

        $ins = $pdo->prepare("INSERT " . " INTO friends (requester_user_id, addressee_user_id, status, created_at, updated_at)
    ...
    {//aceptar solicitud de amistad
        ...
        // Solo puede aceptar quien es addressee del request
    ...
    public static function deleteFriend(int $friendRowId): void
    {
        ...
        // Solo puede borrar quien es parte de esa relación y está aceptada
        $st = $pdo->prepare("SELECT * " . " FROM friends WHERE id = :id AND deleted_at IS NULL LIMIT 1 ");
